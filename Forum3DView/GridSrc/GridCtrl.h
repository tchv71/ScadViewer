...
public:
	...
	inline CSize GetCellTextExtent(int nRow, int nCol) const
	{
...
public:
	...
	const CGridCellBase* GetConstCell(int nRow, int nCol) const; // Get the actual cell!
...
public:
	...
	{
		m_pfnVirtualCompare = VirtualCompare;
	}
...
{
	if (bFixedRow && bFixedCol) return reinterpret_cast<CGridCellBase*>(&m_cellFixedRowColDef);
	if (bFixedRow) return reinterpret_cast<CGridCellBase*>(&m_cellFixedRowDef);
	if (bFixedCol) return reinterpret_cast<CGridCellBase*>(&m_cellFixedColDef);
	return reinterpret_cast<CGridCellBase*>(&m_cellDefault);
}

inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol)
{
	...
	if (GetVirtualMode())
	{
		...
		return static_cast<CGridCellBase*>(&cell);
	}
...
inline const CGridCellBase* CGridCtrl::GetConstCell(int nRow, int nCol) const
{
	return const_cast<const CGridCellBase*>(const_cast<CGridCtrl*>(this)->GetCell(nRow, nCol));
}
