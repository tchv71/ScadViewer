...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	else if (nChar == VK_RIGHT || (nChar == VK_TAB && !IsSHIFTpressed()))
	{
		...
		{
			CDialog* p = static_cast<CDialog*>(GetParent());
			if (p) p->NextDlgCtrl();
			return;
		}
...
{
	GV_DISPINFO* pgvDispInfo = reinterpret_cast<GV_DISPINFO *>(pNMHDR);
...
BOOL CGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = nullptr */)
{
	...
	if (pDC)
	{
		...
		else
		{
			...
			CPen* pOldPen = static_cast<CPen*>(pDC->SelectObject(&pen));
			if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_HORZ)
			{
                                 ...
                                 BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
	...
	if (bSelectCells)
	{
		...
		for (pos = m_SelectedCellMap.GetStartPosition(); pos != nullptr;)
		{
			DWORD key;
			CCellID cell;
			m_SelectedCellMap.GetNextAssoc(pos, key, static_cast<CCellID&>(cell));
			...
			nMinRow >= 0 && nMinCol >= 0 && nMaxRow >= 0 && nMaxCol >= 0)
		{
			for (pos = m_PrevSelectedCellMap.GetStartPosition(); pos != nullptr; /* nothing */)
			{
				DWORD key;
				CCellID cell;
				m_PrevSelectedCellMap.GetNextAssoc(pos, key, static_cast<CCellID&>(cell));
                                ...
                                BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	CMemFile sf(static_cast<BYTE*>(::GlobalLock(hmem)), UINT(::GlobalSize(hmem)));
...
BOOL CGridCtrl::SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass)
{
	...
	CGridCellBase* pNewCell = static_cast<CGridCellBase*>(pRuntimeClass->CreateObject());
...
CGridCellBase* CGridCtrl::CreateCell(int nRow, int nCol)
{
	...
	CGridCellBase* pCell = static_cast<CGridCellBase*>(m_pRtcDefault->CreateObject());
	if (!pCell)
		return nullptr;
...
int CALLBACK CGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	CGridCellBase* pCell1 = reinterpret_cast<CGridCellBase*>(lParam1);
	CGridCellBase* pCell2 = reinterpret_cast<CGridCellBase*>(lParam2);
...
BOOL CGridCtrl::GetItem(GV_ITEM* pItem) const
{
	if (!pItem)
		return FALSE;
	const CGridCellBase* pCell = GetConstCell(pItem->row, pItem->col);
	if (!pCell)
		return FALSE;
...
{
	const CGridCellBase* pCell = GetConstCell(nRow, nCol);
	if (!pCell)
		return LPARAM(0);
...
BOOL CGridCtrl::SetItemState(int nRow, int nCol, UINT state)
{
	...
	if (bSelected && !(state & GVIS_SELECTED))
	{
		...
		if (m_SelectedCellMap.Lookup(key, static_cast<CCellID&>(cell)))
...
COLORREF CGridCtrl::GetItemBkColour(int nRow, int nCol)
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
COLORREF CGridCtrl::GetItemFgColour(int nRow, int nCol)
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
void CGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
	...
	if (nDifference > 0)
	{
		...
		for (int nCount = 0, col1 = nFirstColumn;
		     (col1 < GetColumnCount()) && (nCount < leftOver); col1++ , nCount++)
		{
			if (m_arColWidths[col1] > 0)
				m_arColWidths[col1] += 1;
		}
	}
	else
	{
...
void CGridCtrl::ExpandRowsToFit(BOOL bExpandFixed /*=TRUE*/)
{
	...
	if (nDifference > 0)
	{
		...
		for (int nCount = 0, row1 = nFirstRow;
		     (row1 < GetRowCount()) && (nCount < leftOver); row1++ , nCount++)
		{
			if (m_arRowHeights[row1] > 0)
				m_arRowHeights[row1] += 1;
		}
	}
	else
	{
...
BOOL CGridCtrl::IsCellSelected(int nRow, int nCol) const
{
	if (GetVirtualMode())
	{
		...
		return m_SelectedCellMap.Lookup(key, static_cast<CCellID&>(cell));
	}
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	else if (m_MouseMode == MOUSE_NOTHING)
	{
		...
		if (IsValid(cell))
		{
			CGridCellBase* p_cell = GetCell(cell.row, cell.col);
			if (p_cell)
				p_cell->OnDblClick(pointClickedRel);
