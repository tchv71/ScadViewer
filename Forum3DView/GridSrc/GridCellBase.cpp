...
const CGridCellBase* CGridCellBase::GetConstDefaultCell() const
{
	if (GetGrid())
		return const_cast<const CGridCellBase*>(GetGrid()->GetDefaultCell(IsFixedRow(), IsFixedCol()));
	return nullptr;
}
...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	const CGridDefaultCell* pDefaultCell = static_cast<const CGridDefaultCell*>(GetDefaultCell());
	if (!pDefaultCell)
		return FALSE;
	...
	if (pGrid->GetSortColumn() == nCol && nRow == 0)
	{
		...
		if (size.cx + rect.left < rect.right)
		{
			...
			if (pGrid->GetSortAscending())
			{
				...
				CPen* pOldPen = static_cast<CPen*>(pDC->SelectObject(&penLight));
...
BOOL CGridCellBase::GetTextRect(LPRECT pRect) const // i/o:  i=dims of cell rect; o=dims of text rect
{
	if (GetImage() >= 0)
	{
		IMAGEINFO Info;
...
CSize CGridCellBase::GetTextExtent(LPCTSTR szText, CDC* pDC /*= nullptr*/) const
{
	CGridCtrl* pGrid = GetGrid();
	ASSERT(pGrid);
	...
	if (pDC == nullptr || szText == nullptr)
	{
		...
		if (pDC == nullptr || szText == nullptr)
		{
			const CGridDefaultCell* pDefCell = static_cast<const CGridDefaultCell*>(GetConstDefaultCell());
			ASSERT(pDefCell);
